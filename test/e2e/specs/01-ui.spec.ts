...
test.describe.serial('moodle', () => {
  ...
  test('enrol-student', async () => {
    ...
    await search.click()
    await search.pressSequentially('Demo', { delay: 80 })
    await page.waitForTimeout(1500)
    console.log('ENROL_OPTIONS ' + JSON.stringify(await page.locator('[role="option"]').allInnerTexts()))
    await shoot(page, 'enrol-modal')
    await page.getByRole('option', { name: /Demo Student/i }).first().click()
    await modal.getByRole('button', { name: /Enrol/ }).click()
    await page.waitForLoadState('networkidle')
